...
void USART1_DMA_Transmit_Init(void){
	...
	// usart1
	DMA_SetConfig(1 ,CHANNEL4 , MEMORY_TO_PERIPHERAL , BYTE, BYTE , ACTIVATED, DEACTIVATED);
...
void USART3_DMA_Transmit_Init(void){
	// usart3
	DMA_SetConfig(1 ,CHANNEL2 , MEMORY_TO_PERIPHERAL , BYTE ,BYTE , ACTIVATED, DEACTIVATED);
...
void USART2_DMA_Receiver_Init(void)
{
	...
	DMA_VoidConfigPriority(1 ,CHANNEL6, MEDIUM);
...
void USART2_DMA_RECEIVE(u8 *RX, u32 Size)
{  
 
	DMA_VoidSetAddress(1 ,CHANNEL6 , (u32) &USART2->DR , (u32 volatile )RX , Size);
  DMA_VoidDMAEnable(1 , CHANNEL6);

}

void USART3_DMA_RECEIVE(u8 *RX, u32 Size)
{
	  
	DMA_VoidSetAddress(1 ,CHANNEL3 , (u32) &USART3->DR , (u32 volatile)RX , Size );
	DMA_VoidDMAEnable(1 , CHANNEL3  );
	
}

void USART1_DMA_RECEIVE(u8 *RX, u32 Size)
{
	DMA_VoidSetAddress(1 ,CHANNEL5 , (u32) &USART1->DR , (u32 volatile )RX , Size );
	DMA_VoidDMAEnable(1 , CHANNEL5  );

}


void USART1_DMA_Transmit(u8 *Tx_data, u32 Size){
 
	SET_BIT(USART1 ->CR1,3);
	DMA_VoidSetAddress(1 ,CHANNEL4 ,(u32)&USART1->DR, (u32 volatile)Tx_data , Size );	
	MNVIC_voidEnableInterrupt(DMA1_Channel4);
	DMA_VoidInerruptEnable(1 , CHANNEL4 , TCIF );
	CLR_BIT(USART1 -> SR ,6); 
	DMA_VoidDMAEnable(1 , CHANNEL4);
}


 void USART2_DMA_Transmit(u8 *Tx_data, u32 Size){
	SET_BIT(USART2 ->CR1,3);
	DMA_VoidSetAddress(1 ,CHANNEL7 ,(u32)&USART2->DR, (u32 volatile)Tx_data , Size );
	MNVIC_voidEnableInterrupt(DMA1_Channel7);
	DMA_VoidInerruptEnable(1 , CHANNEL7 , TCIF );
	CLR_BIT(USART2 -> SR ,6); 
	DMA_VoidDMAEnable(1 , CHANNEL7);
} 

void USART3_DMA_Transmit(u8 *Tx_data, u32 Size){
 
	SET_BIT(USART2 ->CR1,3);
	DMA_VoidSetAddress(1 ,CHANNEL2 ,(u32)&USART3->DR, (u32 volatile)Tx_data , Size );	
	MNVIC_voidEnableInterrupt(DMA1_Channel2);
	DMA_VoidInerruptEnable(1 , CHANNEL2 , TCIF );
	CLR_BIT(USART3 -> SR ,6); 
	DMA_VoidDMAEnable(1 , CHANNEL2);
}

void DMA1_Channel7_IRQHandler(){
	MDMA_VoidDMADisable(1, CHANNEL7);
...
void DMA1_Channel2_IRQHandler(){
	MDMA_VoidDMADisable(1, CHANNEL2);
...
void DMA1_Channel4_IRQHandler(){
	MDMA_VoidDMADisable(1, CHANNEL4);
