...
void DMA_VoidClearCircularMode(u8 Copy_u8Dma ,u8 Copy_u8CHANNEL)
{
	switch(Copy_u8Dma)
    {
    case 1 :
	 CLR_BIT( DMA1->CHANNEL[Copy_u8CHANNEL].CCR , 5  );//enable circular mode  
	 break;
    case 2 :
	 CLR_BIT( DMA2->CHANNEL[Copy_u8CHANNEL].CCR , 5  );//enable circular mode 
		break;
		default : break;
    }
}
...
u8 index=0;
u8 DMA_USART_RX_IRQHandler(u8 Copy_u8Dma, u8 DATA[], u8 DATA_SIZE , u8 MAIN[50])
	{
		switch(Copy_u8Dma)
		{
			case 1:
				if((DMA1->ISR)& (1<<22))
				{
					memcpy(&MAIN[index],&DATA[0],DATA_SIZE/2);
					DMA1->IFCR |= (1<<22);
					index= index+(DATA_SIZE/2);
					if (index>49) index=0;
				}
				break;
			case 2:
