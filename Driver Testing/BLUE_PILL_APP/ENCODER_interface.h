...
void ENCODER_Define (encoder_t *name, u8 PinSet);//Used to map a pin set to a defined encoder type variable and initialize encoder
u32 ENCODER_CurrentPosition(encoder_t *encoder); //Return the current position (in degrees)
float ENCODER_CurrentSpeed(encoder_t *encoder);    //Return the current speed of the encoder (in rpm)
...
/*Steps to use the driver:
	...
	2- Map the encoder pins using ENCODER_Define with your encoder instance as a parameter and pinset
...
#define ENC_PINS_4    4      //{PWM_PA0,PWM_PA1,PWM_PA8}
