...
********************************************************/
   ...
   ADC_RCC_PRE &= ~(3<<14);          //Reset Prescaler bits
   #if ADC_PRESCALER==2 
	   ADC_RCC_PRE &= ~(3<<14);      //PRESCALER BITS (0,0)
   
   #elif ADC_PRESCALER==4 
	   ADC_RCC_PRE |=  (1<<14);      //PRESCALER BITS (0,1)
   
   #elif ADC_PRESCALER==6 
	   ADC_RCC_PRE |=  (2<<14);      //PRESCALER BITS (1,0)
   
   #elif ADC_PRESCALER==8 
	   ADC_RCC_PRE |=  (3<<14);      //PRESCALER BITS (1,1)
   ...
   //Setting ADC1 Sample Time (Cycles)
	   ...
	   ADC1_SMPR1 &= ~(0x00FFFFFFUL); //SMP BITS OF CHNLS 10 -> 18 are (0,0,0)
	   ADC1_SMPR2 &= ~(0x3FFFFFFFUL); //SMP BITS OF CHNLS 0 -> 9 are (0,0,0)
   
   #elif ADC_SAMPLING_CYCLES==15
	   for(u8 i=0;i<8;i++){       //SMP BITS OF CHNLS 10 -> 18 are (0,0,1)
	   ...
	   for(u8 i=0;i<8;i++){       //SMP BITS OF CHNLS 10 -> 18 are (0,1,0)
	   ADC1_SMPR1 &= ~((0xUL)<<(i*3)); 
	   ...
	   for(u8 i=0;i<8;i++){       //SMP BITS OF CHNLS 10 -> 18 are (0,1,1)
	   ...
	   for(u8 i=0;i<8;i++){       //SMP BITS OF CHNLS 10 -> 18 are (1,0,0)
	   ADC1_SMPR1 &= ~((0x3UL)<<(i*3)); 
       ADC1_SMPR1 |= ((0x4UL)<<(i*3)); 
	   }
	   ...
	   for(u8 i=0;i<8;i++){       //SMP BITS OF CHNLS 10 -> 18 are (1,0,1)
	   ADC1_SMPR1 &= ~((0x2UL)<<(i*3)); 
	   ...
	   for(u8 i=0;i<8;i++){       //SMP BITS OF CHNLS 10 -> 18 are (1,1,0)
	   ADC1_SMPR1 &= ~((0x1UL)<<(i*3)); 
       ADC1_SMPR1 |= ((0x6UL)<<(i*3)); 
	   }
	   ...
	   ADC1_SMPR1 |= (0x00FFFFFFUL); //SMP BITS OF CHNLS 10 -> 18 are (1,1,1)
   ...
   //Discontnuous Mode
	...
	#endif  
	//Set ADC to start conversion with software trigger
	ADC1_CR2 |= (1<<20);   //EXTRIG= 1
	ADC1_CR2 |= (7<<17);   //EXTSEL= SWSTRT
	ADC1_CR2 |= (7<<12);   //JEXTSEL= JSTWRT
